...
describe("IDRPController", function () {
  ...
  async function deployFixture() {
    const [admin, officer, manager, director, commissioner, user, depository] = await hre.ethers.getSigners()
  ...
  describe("Withdrawal", function () {
    it("Should allow withdrawal of other tokens", async function () {
      const { controller, depository, admin } = await loadFixture(deployFixture)
    ...
    it("Should not allow withdrawal of IDRP token", async function () {
      const { controller, idrp, admin, depository } = await loadFixture(deployFixture)
      ...
      await idrp.connect(depository).transfer(await controller.getAddress(), hre.ethers.parseUnits("100", 6))
