...
describe("IDRPController", function () {
  ...
  async function deployFixture() {
    const [admin, officer, manager, director, commissioner, user] = await hre.ethers.getSigners()
  ...
  describe("Signature Verification", function () {
    ...
    it("Should execute operation with proper signatures - large amount", async function () {
      ...
      const officerSignature = await officer.signTypedData(domain, types, operation)
      const managerSignature = await manager.signTypedData(domain, types, operation)
      const directorSignature = await director.signTypedData(domain, types, operation)
      const commissionerSignature = await commissioner.signTypedData(domain, types, operation)
  ...
  describe("Token Operations", function () {
    ...
    it("Should execute burn operation", async function () {
      ...
      const officerMintSignature = await officer.signTypedData(domain, types, mintOperation)
      ...
      await idrp.connect(user).approve(await controller.getAddress(), burnAmount)
      ...
      const officerBurnSignature = await officer.signTypedData(domain, types, burnOperation)
      ...
      expect(await idrp.balanceOf(user.address)).to.equal(mintAmount - burnAmount)
    })

    it("Should execute freeze operation", async function () {
      const { controller, idrp, admin, officer, user, domain, types } = await loadFixture(deployFixture)
      ...
      const officerFreezeSignature = await officer.signTypedData(domain, types, freezeOperation)
      await controller.executeOperation(
      ...
      await expect(idrp.connect(user).transfer(admin.address, hre.ethers.parseUnits("1000", 6))).to.be.rejected
    })
  })
  ...
  describe("Withdrawal", function () {
    it("Should allow withdrawal of other tokens", async function () {
      ...
      await testToken.connect(admin).mint(admin.address, hre.ethers.parseUnits("1000", 6))

      // Transfer some tokens to the controller
      const transferAmount = hre.ethers.parseUnits("100", 6)
      await testToken.connect(admin).transfer(await controller.getAddress(), transferAmount)

      // Verify controller has the tokens
      expect(await testToken.balanceOf(await controller.getAddress())).to.equal(transferAmount)

      // Admin withdraws the tokens
      await controller.connect(admin).withdrawToken(await testToken.getAddress(), admin.address, transferAmount)

      // Verify balances after withdrawal
      expect(await testToken.balanceOf(await controller.getAddress())).to.equal(0)
      expect(await testToken.balanceOf(admin.address)).to.equal(hre.ethers.parseUnits("1000", 6))
    })
    ...
    it("Should not allow withdrawal of IDRP token", async function () {
      ...
      await idrp.connect(admin).mint(await controller.getAddress(), hre.ethers.parseUnits("100", 6))

      // Attempt to withdraw IDRP tokens, should fail
      await expect(
        controller.connect(admin).withdrawToken(await idrp.getAddress(), admin.address, hre.ethers.parseUnits("100", 6))
      ).to.be.revertedWith("Cannot withdraw IDRP token")

      // Verify tokens still in controller
      expect(await idrp.balanceOf(await controller.getAddress())).to.equal(hre.ethers.parseUnits("100", 6))
    })
  })
})
